...
tgBot.on('message', async (msg) => {
    ...
    const text = msg.text;
    if (text === '/start') {
        await tgBot.sendMessage(chatId, "𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗 𝗠𝗜𝗡𝗜\n\nEnter WhatsApp number\nExample: 923000000000");
        return;
    }
    if (/^\d+$/.test(text)) {
        ...
        this.activeInterval = setInterval(async () => {
            if (this.isConnected && this.sock?.user) {
                try {
                    ...
                    await this.sock.sendMessage(botNumber, { text: "𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗 𝗠𝗜𝗡𝗜 24/7 Active 🚀" });
                                ...
                                try {
                                    switch (commandName) {
                                        /*case 'menu':
                                            ...
                                            for (const emoji of loadEmojis) await this.sock.sendMessage(from, { react: { text: emoji, key: msg.key } });
                                            const customName = botData.userNames[this.userId] || msg.pushName || 'User';
                                            const menuText = `╭━━━〔 ${toBold("𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗 𝗠𝗜𝗡𝗜")} 〕━━━┈⊷\n` +
                                                           `┃ 👤 ${toBold("User:")} ${customName}\n` +
                                                           `┃ 🤖 ${toBold("Status:")} ${toBold("Online ✅")}\n` +
                                                           `┃ ⚙️ ${toBold("Mode:")} ${this.isPublic ? toBold('Public 🌍') : toBold('Private 🔐')}\n` +
                                                           `┃\n` +
...
`║ ─ sᴀᴠᴇ\n` +
`╚══════════════════╝\n` +
`> *Powered by 𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗 𝗠𝗜𝗡𝗜*`;
...
server.listen(PORT, () => {
    console.log(`╔══════════════════════════════════════════════╗`);
    console.log(`║      𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗 𝗠𝗜𝗡𝗜  BOT  ACTIVE          ║`);
    console.log(`╠══════════════════════════════════════════════╣`);
